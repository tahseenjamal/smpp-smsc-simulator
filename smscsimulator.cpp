...
class DeliverQueue {
   public:
    ...
    void add(const string& id, uint64_t when) { items.push_back({id, when}); }
...
class SMPPSession {
   ...
   public:
    ...
    bool readPDU(uint32_t& cmdid, uint32_t& seq) {
        ...
        if (!recvAll(sock, header, 16)) return false;
        ...
        if (len > 16) {
            ...
            if (!recvAll(sock, body.data(), len - 16)) return false;
        }
    ...
    bool run() {
        ...
        if (select(sock + 1, &rfds, nullptr, nullptr, &tv) <= 0) return false;

        uint32_t cmdid, seq;

        if (!readPDU(cmdid, seq)) return true;
        ...
        if (cmdid == 0x00000004)  // SubmitSM
        {
            ...
            *(uint32_t*)(resp + 12) = htonl(seq);
            ...
            cout << "Submit OK\n";
        } else if (cmdid == 0x00000015)  // EnquireLink
        {
            uint8_t resp[16];
    ...
    bool timed() {
        ...
        while (queue.pop(item)) {
            ...
            string receipt = "id:" + item.msgid +
                             " sub:001 dlvrd:001 submit date:2402241200 done "
                             "date:2402241205 "
                             "stat:DELIVRD err:000 text:HelloWorldTest12";

            pdu[idx++] = receipt.length() + 1;
            memcpy(pdu + idx, receipt.c_str(), receipt.length() + 1);
            idx += receipt.length() + 1;

            *(uint32_t*)(pdu + 0) = htonl(idx);
            *(uint32_t*)(pdu + 4) = htonl(0x00000005);
            *(uint32_t*)(pdu + 8) = htonl(0);
            *(uint32_t*)(pdu + 12) = htonl(seqOut++);

            if (!sendAll(sock, pdu, idx)) return true;
        }
