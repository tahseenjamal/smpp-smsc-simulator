// main.cpp
// High TPS SMPP Simulator with DLR (Linux epoll)
...
int main() {
    ...
    std::cout << "High TPS SMPP Server with DLR on 2775 (epoll)\n";

    while (true) {
        // 5ms tick (same as kqueue timespec)
        int nev = epoll_wait(epfd, events, MAX_EVENTS, 5);
